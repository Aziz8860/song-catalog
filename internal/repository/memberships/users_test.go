...
func Test_repository_CreateUser(t *testing.T) {
	...
	assert.NoError(t, err) // mastiin gaada error ketika inisiasi gorm atau sqlmock
	...
	}{
		{
			...
			mockFn: func(args args) {
				...
				mock.ExpectQuery(`INSERT INTO "users" (.+) VALUES (.+)`).
					WithArgs(
						sqlmock.AnyArg(), // created ay
						sqlmock.AnyArg(), // updated at
						sqlmock.AnyArg(), // deleted at
						args.model.Email,
						args.model.Username,
						args.model.Password,
...
func Test_repository_GetUser(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantErr: false,
			mockFn: func(args args) { // pake backslash untuk bintang
